...
public class VideosGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    List<Video> videos;
    Context context;
    ...
    private class AsyncLoadThumbs extends AsyncTask<VideoAndView, Void, VideoAndView> {
        ...
        protected final VideoAndView doInBackground(VideoAndView... args) {
            ...
            container.thumbnail = ExtractThumbUtility.getThumbnailPathForLocalFile(VideosGridAdapter.this.context, container.uri);
            return container;
        }
        ...
        protected void onPostExecute(VideoAndView result) {
            ...
            if (!isCancelled()) {
                ...
                Picasso.with(VideosGridAdapter.this.context).load(thumbnailUri).into(result.imageView);
            }
        }
    }
    ...
    public VideosGridAdapter(List<Video> videos, Context context) {
        this.videos = videos;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_grid_view, parent, false);
