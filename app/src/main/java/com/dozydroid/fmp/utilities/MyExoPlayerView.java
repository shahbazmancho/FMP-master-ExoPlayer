 ...
 */
@TargetApi(16)
public final class MyExoPlayerView extends FrameLayout implements View.OnTouchListener {
    ...
    private final MyExoPlayerView.ComponentListener componentListener;
    ...
    public MyExoPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        componentListener = new MyExoPlayerView.ComponentListener();
            ...
            try {
                ...
                if (Math.abs(diffX) > Math.abs(diffY)) {
                    if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                        if (diffX > 0) {
                            onSwipeRight((int)diffX);
                        } else {
                            onSwipeLeft((int) diffX);
                        }
                        result = true;
                    }
                }
